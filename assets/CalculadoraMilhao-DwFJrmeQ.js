import{r as t,j as e}from"./index-CA5Ttaus.js";import{C as S,a as E}from"./CalculadoraTextBlock-DQ1k9L_E.js";const w=r=>parseFloat(r.replace(/\./g,"").replace(",","."))||0,v=r=>r.toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2}),N=r=>{const o=r.replace(/\D/g,"");return o?(parseInt(o,10)/100).toLocaleString("pt-BR",{minimumFractionDigits:2}):""};function W({label:r,value:o,accent:l,delay:p=0,visible:i}){return e.jsxs("div",{className:"result-card",style:{borderLeft:`4px solid ${l}`,transitionDelay:`${p}ms`,opacity:i?1:0,transform:i?"translateY(0)":"translateY(12px)",transition:"opacity 0.45s ease, transform 0.45s ease"},children:[e.jsx("p",{className:"result-label",children:r}),e.jsxs("p",{className:"result-value",children:["R$ ",o]})]})}function L(){const[r,o]=t.useState("1.000.000,00"),[l,p]=t.useState(""),[i,y]=t.useState(""),[u,k]=t.useState("Anos"),[x,C]=t.useState(""),[s,q]=t.useState(null),[d,g]=t.useState(!1),h=t.useRef(null),R=()=>{const a=w(r),n=w(l),z=(parseFloat(x.replace(",","."))||0)/100,b=parseInt(i,10)||0,c=u==="Anos"?b*12:b;if(!a||!c)return;const f=Math.pow(1+z,1/12)-1;let m=0;if(f===0)m=Math.max(0,(a-n)/c);else{const j=Math.pow(1+f,c);m=Math.max(0,(a-n*j)/((j-1)/f))}const M=m*c+n,F=Math.max(0,a-M);q({monthlyPayment:m,totalSaved:a,interestEarned:F}),g(!1),setTimeout(()=>g(!0),50)};return t.useEffect(()=>{s&&h.current&&window.innerWidth<768&&setTimeout(()=>{var n;return(n=h.current)==null?void 0:n.scrollIntoView({behavior:"smooth",block:"start"})},100)},[s]),e.jsxs(e.Fragment,{children:[e.jsx("style",{children:`
