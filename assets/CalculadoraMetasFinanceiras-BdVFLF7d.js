import{r,j as e}from"./index-DLXXpc68.js";import{C as S,a as A}from"./CalculadoraTextBlock-Ba_seZLi.js";const k=a=>parseFloat(a.replace(/\./g,"").replace(",","."))||0,f=a=>a.toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2}),y=a=>{const s=a.replace(/\D/g,"");return s?(parseInt(s,10)/100).toLocaleString("pt-BR",{minimumFractionDigits:2}):""};function v({label:a,value:s,accent:n,delay:m=0,visible:l}){return e.jsxs("div",{className:"result-card",style:{borderLeft:`4px solid ${n}`,transitionDelay:`${m}ms`,opacity:l?1:0,transform:l?"translateY(0)":"translateY(12px)",transition:"opacity 0.45s ease, transform 0.45s ease"},children:[e.jsx("p",{className:"result-label",children:a}),e.jsxs("p",{className:"result-value",children:["R$ ",s]})]})}function L(){const[a,s]=r.useState(""),[n,m]=r.useState(""),[l,N]=r.useState(""),[x,C]=r.useState("Anos"),[g,z]=r.useState("14"),[o,q]=r.useState(null),[p,b]=r.useState(!1),u=r.useRef(null),E=()=>{const t=k(a),i=k(n),R=(parseFloat(g.replace(",","."))||0)/100,j=parseInt(l,10)||0,d=x==="Anos"?j*12:j;if(!t||!d)return;const h=Math.pow(1+R,1/12)-1;let c=0;if(h===0)c=Math.max(0,(t-i)/d);else{const w=Math.pow(1+h,d);c=Math.max(0,(t-i*w)/((w-1)/h))}const F=c*d+i,M=Math.max(0,t-F);q({monthlyPayment:c,totalSaved:t,interestEarned:M}),b(!1),setTimeout(()=>b(!0),50)};return r.useEffect(()=>{o&&u.current&&window.innerWidth<768&&setTimeout(()=>{var i;return(i=u.current)==null?void 0:i.scrollIntoView({behavior:"smooth",block:"start"})},100)},[o]),e.jsxs(e.Fragment,{children:[e.jsx("style",{children:`
        .vt-root {
