import{r as o,j as e}from"./index-Aq57YJWh.js";const w=s=>parseFloat(s.replace(/\./g,"").replace(",","."))||0,v=s=>s.toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2}),N=s=>{const i=s.replace(/\D/g,"");return i?(parseInt(i,10)/100).toLocaleString("pt-BR",{minimumFractionDigits:2}):""};function f({label:s,value:i,accent:n,delay:m=0,visible:l}){return e.jsxs("div",{className:"result-card",style:{borderLeft:`4px solid ${n}`,transitionDelay:`${m}ms`,opacity:l?1:0,transform:l?"translateY(0)":"translateY(12px)",transition:"opacity 0.45s ease, transform 0.45s ease"},children:[e.jsx("p",{className:"result-label",children:s}),e.jsxs("p",{className:"result-value",children:["R$ ",i]})]})}function W(){const[s,i]=o.useState(""),[n,m]=o.useState(""),[l,C]=o.useState(""),[g,z]=o.useState("Anos"),[j,F]=o.useState(""),[t,q]=o.useState(null),[p,b]=o.useState(!1),u=o.useRef(null),M=()=>{const r=w(s),a=w(n),x=(parseFloat(j.replace(",","."))||0)/100,y=parseInt(l,10)||0,d=g==="Anos"?y*12:y;if(!r||!d)return;const h=Math.pow(1+x,1/12)-1;let c=0;if(h===0)c=Math.max(0,(r-a)/d);else{const k=Math.pow(1+h,d);c=Math.max(0,(r-a*k)/((k-1)/h))}const R=c*d+a,E=Math.max(0,r-R);q({monthlyPayment:c,totalSaved:r,interestEarned:E}),b(!1),setTimeout(()=>b(!0),50)};return o.useEffect(()=>{t&&u.current&&window.innerWidth<768&&setTimeout(()=>{var a;return(a=u.current)==null?void 0:a.scrollIntoView({behavior:"smooth",block:"start"})},100)},[t]),e.jsxs(e.Fragment,{children:[e.jsx("style",{children:`
