import{r as t,j as e,S as q,O as A}from"./index-BLcz9e82.js";import{C as E,a as W}from"./CalculadoraTextBlock-CoN6yLp2.js";const w=a=>parseFloat(a.replace(/\./g,"").replace(",","."))||0,v=a=>a.toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2}),y=a=>{const o=a.replace(/\D/g,"");return o?(parseInt(o,10)/100).toLocaleString("pt-BR",{minimumFractionDigits:2}):""};function D({label:a,value:o,accent:l,delay:p=0,visible:i}){return e.jsxs("div",{className:"result-card",style:{borderLeft:`4px solid ${l}`,transitionDelay:`${p}ms`,opacity:i?1:0,transform:i?"translateY(0)":"translateY(12px)",transition:"opacity 0.45s ease, transform 0.45s ease"},children:[e.jsx("p",{className:"result-label",children:a}),e.jsxs("p",{className:"result-value",children:["R$ ",o]})]})}function B(){const[a,o]=t.useState("1.000.000,00"),[l,p]=t.useState(""),[i,N]=t.useState(""),[u,k]=t.useState("Anos"),[g,C]=t.useState(""),[s,R]=t.useState(null),[d,x]=t.useState(!1),h=t.useRef(null),F=()=>{const r=w(a),n=w(l),S=(parseFloat(g.replace(",","."))||0)/100,b=parseInt(i,10)||0,c=u==="Anos"?b*12:b;if(!r||!c)return;const f=Math.pow(1+S,1/12)-1;let m=0;if(f===0)m=Math.max(0,(r-n)/c);else{const j=Math.pow(1+f,c);m=Math.max(0,(r-n*j)/((j-1)/f))}const z=m*c+n,M=Math.max(0,r-z);R({monthlyPayment:m,totalSaved:r,interestEarned:M}),x(!1),setTimeout(()=>x(!0),50)};return t.useEffect(()=>{s&&h.current&&window.innerWidth<768&&setTimeout(()=>{var n;return(n=h.current)==null?void 0:n.scrollIntoView({behavior:"smooth",block:"start"})},100)},[s]),e.jsxs(e.Fragment,{children:[e.jsx("style",{children:`
        .vt-root {
