import{r,j as e,S,O as A}from"./index-BLcz9e82.js";import{C as W,a as D}from"./CalculadoraTextBlock-CoN6yLp2.js";const k=t=>parseFloat(t.replace(/\./g,"").replace(",","."))||0,f=t=>t.toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2}),y=t=>{const s=t.replace(/\D/g,"");return s?(parseInt(s,10)/100).toLocaleString("pt-BR",{minimumFractionDigits:2}):""};function v({label:t,value:s,accent:n,delay:m=0,visible:l}){return e.jsxs("div",{className:"result-card",style:{borderLeft:`4px solid ${n}`,transitionDelay:`${m}ms`,opacity:l?1:0,transform:l?"translateY(0)":"translateY(12px)",transition:"opacity 0.45s ease, transform 0.45s ease"},children:[e.jsx("p",{className:"result-label",children:t}),e.jsxs("p",{className:"result-value",children:["R$ ",s]})]})}function B(){const[t,s]=r.useState(""),[n,m]=r.useState(""),[l,N]=r.useState(""),[x,C]=r.useState("Anos"),[g,z]=r.useState("14"),[o,q]=r.useState(null),[p,b]=r.useState(!1),u=r.useRef(null),F=()=>{const a=k(t),i=k(n),R=(parseFloat(g.replace(",","."))||0)/100,j=parseInt(l,10)||0,d=x==="Anos"?j*12:j;if(!a||!d)return;const h=Math.pow(1+R,1/12)-1;let c=0;if(h===0)c=Math.max(0,(a-i)/d);else{const w=Math.pow(1+h,d);c=Math.max(0,(a-i*w)/((w-1)/h))}const E=c*d+i,M=Math.max(0,a-E);q({monthlyPayment:c,totalSaved:a,interestEarned:M}),b(!1),setTimeout(()=>b(!0),50)};return r.useEffect(()=>{o&&u.current&&window.innerWidth<768&&setTimeout(()=>{var i;return(i=u.current)==null?void 0:i.scrollIntoView({behavior:"smooth",block:"start"})},100)},[o]),e.jsxs(e.Fragment,{children:[e.jsx("style",{children:`
        .vt-root {
