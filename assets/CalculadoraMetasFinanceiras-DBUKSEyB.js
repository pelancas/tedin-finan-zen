import{r as s,j as e}from"./index-BGIv2ayS.js";import{C as S,a as A}from"./CalculadoraTextBlock-CWPqrIl7.js";const N=r=>parseFloat(r.replace(/\./g,"").replace(",","."))||0,x=r=>r.toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2}),y=r=>{const t=r.replace(/\D/g,"");return t?(parseInt(t,10)/100).toLocaleString("pt-BR",{minimumFractionDigits:2}):""};function v({label:r,value:t,accent:n,delay:m=0,visible:l}){return e.jsxs("div",{className:"result-card",style:{borderLeft:`4px solid ${n}`,transitionDelay:`${m}ms`,opacity:l?1:0,transform:l?"translateY(0)":"translateY(12px)",transition:"opacity 0.45s ease, transform 0.45s ease"},children:[e.jsx("p",{className:"result-label",children:r}),e.jsxs("p",{className:"result-value",children:["R$ ",t]})]})}function P(){const[r,t]=s.useState(""),[n,m]=s.useState(""),[l,k]=s.useState(""),[f,C]=s.useState("Anos"),[g,q]=s.useState(""),[o,F]=s.useState(null),[p,j]=s.useState(!1),u=s.useRef(null),R=()=>{const a=N(r),i=N(n),z=(parseFloat(g.replace(",","."))||0)/100,b=parseInt(l,10)||0,d=f==="Anos"?b*12:b;if(!a||!d)return;const h=Math.pow(1+z,1/12)-1;let c=0;if(h===0)c=Math.max(0,(a-i)/d);else{const w=Math.pow(1+h,d);c=Math.max(0,(a-i*w)/((w-1)/h))}const E=c*d+i,M=Math.max(0,a-E);F({monthlyPayment:c,totalSaved:a,interestEarned:M}),j(!1),setTimeout(()=>j(!0),50)};return s.useEffect(()=>{o&&u.current&&window.innerWidth<768&&setTimeout(()=>{var i;return(i=u.current)==null?void 0:i.scrollIntoView({behavior:"smooth",block:"start"})},100)},[o]),e.jsxs(e.Fragment,{children:[e.jsx("style",{children:`
